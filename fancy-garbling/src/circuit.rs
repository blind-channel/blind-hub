...
impl std::fmt::Display for Gate {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Gate::Constant { val, out } => write!(f, "Constant {}, {:?}", val, out),
...
impl Circuit {
    ...
    pub fn eval_label<F: Fancy>(
        ...
        for (i, gate) in self.gates.iter().enumerate() {
            ...
            let (zref_, val) = match *gate {
                ...
                Gate::Constant { val, out } => (out, f.constant(val, q)?),
    ...
    pub fn eval_from_label<F: Fancy>(
        &self,
        f: &mut F,
        output_label: Vec<F::Item>
    ) -> Result<Option<Vec<u16>>, F::Error>{
        let mut outputs = Vec::with_capacity(self.output_refs.len());
        for r in output_label {
            outputs.push(f.output(&r)?);
        }
    ...
    pub fn eval<F: Fancy>(
        ...
        let output_label = self.eval_label(f, garbler_inputs, evaluator_inputs)?;
        self.eval_from_label(f,output_label)
    }
    ...
    pub fn get_output_ref(&self) -> &[CircuitRef]{
        self.output_refs.as_slice()
    }
}
...
impl Fancy for CircuitBuilder {
    ...
    fn constant(&mut self, val: u16, modulus: u16) -> Result<CircuitRef, Self::Error> {
        match self.const_map.get(&(val, modulus)) {
            Some(&r) => Ok(r),
            None => {
                let gate = Gate::Constant { val, out: None };
