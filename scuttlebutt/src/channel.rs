...
pub trait AbstractChannel {
    ...
    #[inline(always)]
    fn write_scalar(&mut self, scalar: &Secp256k1Scalar) -> Result<()>{
        let scalar_byte: [u8;32] = scalar.serialize().try_into().unwrap();
        self.write_bytes(&scalar_byte)?;
        Ok(())
    }

    #[inline(always)]
    fn read_scalar(&mut self) -> Result<Secp256k1Scalar> {
        let mut data = [0u8;32];
        self.read_bytes(&mut data)?;
        Ok(Secp256k1Scalar::deserialize(&data).unwrap())
    }

    /// Write a `RistrettoPoint` to the channel.
    #[inline(always)]
    fn write_pt(&mut self, pt: &Secp256k1Point) -> Result<()> {
        let point_byte: [u8;33] = pt.serialize_compressed().as_slice().try_into().unwrap();
        self.write_bytes(&point_byte)?;
        Ok(())
    }

    /// Read a `RistrettoPoint` from the channel.
    #[inline(always)]
    fn read_pt(&mut self) -> Result<Secp256k1Point> {
        let mut data = [0u8; 33];
        self.read_bytes(&mut data)?;
        let pt = match Secp256k1Point::deserialize(&data) {
            Ok(pt) => pt,
            Err(_) => {
